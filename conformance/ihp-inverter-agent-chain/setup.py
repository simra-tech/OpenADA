#!/usr/bin/env python3
"""Fetch the exact public design and OCI image used by the agent chain."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fetch the pinned IHP inverter and IIC-OSIC-TOOLS image."
    )
...
def fetch_design(design_dir: Path, manifest: dict) -> None:
    ...
    temporary = Path(
        tempfile.mkdtemp(prefix=".ihp-agent-chain-fetch-", dir=design_dir.parent)
    )
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Pulling pinned {image['platform']} image {image['image_reference']} ...")
        run_checked(
            [
        ...
        print(f"Fetching pinned design revision into {design_dir} ...")
