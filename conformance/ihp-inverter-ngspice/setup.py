#!/usr/bin/env python3
"""Fetch the pinned external inputs for IHP Xschem/ngspice conformance."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Pull the pinned IIC image and fetch the pinned IHP inverter design."
    )
...
def fetch_design(design_dir: Path, manifest: dict) -> None:
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".ihp-ngspice-fetch-", dir=design_dir.parent))
    try:
