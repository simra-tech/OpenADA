#!/usr/bin/env python3
"""Fetch the pinned image and ORFS design checkout for Ibex conformance."""
...
def fetch_design(design_dir: Path, manifest: dict) -> None:
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".orfs-ibex-fetch-", dir=design_dir.parent))
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        design_dir = ensure_external_design_path(
            cache_dir / "OpenROAD-flow-scripts", REPOSITORY_ROOT, cache_dir
        )
        verify_derived_inputs(REPOSITORY_ROOT, manifest)
        ...
        print(f"Fetching pinned ORFS revision into {design_dir} ...", flush=True)
