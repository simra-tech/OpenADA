#!/usr/bin/env python3
"""Fetch the exact public IHP revision and pinned OCI image."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fetch the pinned IHP measurement inputs.")
...
def fetch_design(design_dir: Path, manifest: dict) -> None:
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".ihp-measurements-", dir=design_dir.parent))
    try:
        ...
        run_checked(
            ["git", "-C", str(temporary), "fetch", "--depth", "1", "origin", manifest["design"]["revision"]]
        )
        run_checked(
            ["git", "-C", str(temporary), "checkout", "--quiet", "--detach", manifest["design"]["revision"]]
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        fetch_design(design, manifest)
    except ConformanceError as exc:
        print(f"setup failed: {exc}", file=sys.stderr)
        return 1
    print("Setup complete; the replay is network-disabled.")
    return 0
