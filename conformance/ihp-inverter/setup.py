#!/usr/bin/env python3
"""Fetch the two pinned external inputs for IHP inverter conformance."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Pull the pinned IIC image and fetch the pinned IHP AnalogAcademy revision."
    )
    ...
    parser.add_argument(
        "--container-engine",
        default="docker",
        help="OCI container CLI (default: docker)",
    )
    return parser
...
def fetch_design(design_dir: Path, manifest: dict) -> None:
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".ihp-inverter-fetch-", dir=design_dir.parent))
    try:
        run_checked(["git", "init", "--quiet", str(temporary)])
        run_checked(
            ["git", "-C", str(temporary), "remote", "add", "origin", design["repository"]]
        )
        run_checked(
            [
                "git",
        ...
        run_checked(
            ["git", "-C", str(temporary), "checkout", "--quiet", "--detach", design["revision"]]
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        ensure_external_design_path(design_dir, REPOSITORY_ROOT, cache_dir)
        verify_design_checkout(design_dir, manifest)
