...
Derive sampling facts from the retained waveform evidence and the
`result.spectral.measure` definition. Keep these values distinct:

- requested simulator step or maximum step;
- actual waveform sample interval and sample rate;
...
A requested maximum step is not an actual waveform sample interval.
Oversampling ratio is not samples per period or FFT length.

## Choose between the implemented and a future method

Before invoking the `result.spectral.measure` contract, declare:
...
choice or mark the `result.spectral.measure` result unknown/not evaluated as
dictated by the contract result.

## Evaluate and compare

Invoke the specification operation only for an explicit numeric bound with
units and conditions. A valid supported `result.spectral.measure` result such as
SFDR or THD without a supplied limit is a simulation result, not a specification
pass. HD2 and HD3 stay not evaluated in this release.
...
| State | Next action |
|---|---|
| Transient evidence pass | Attempt only spectral metric kinds advertised by the installed profile |
| Series extraction pass | Confirm the actual waveform sample interval, record length, signal mode, units, and canonical series digest |
...
| `result.spectral.measure` pass | Report the metric with its complete sampling and bin ledger |
| `result.spectral.measure` fail/unknown | Correct the declared method or source evidence; do not use a favorable plot scalar |
| `result.spectral.measure` capability unavailable | Keep the waveform valid but the metric not evaluated; propose the smallest missing primitive |
...
smallest next experiment. Separate **simulation results** from **inferred**
causes and **proposed** changes. Finish with `signoff: not claimed`.
