...
- **Evidence:** target and configuration identities, prior request/result IDs,
  artifact hashes, simulation-result definitions, and known provenance gaps.
- **Unknowns:** assumptions whose resolution could change the testbench,
  analysis, simulation result, or conclusion.
...
to inspect the installed parameter schema before constructing result-operation
or specification requests. Use semantic metric names in the characterization
plan, but do not invent result-operation parameter names, backend expressions,
or feature IDs that the installed profile does not define.
...
| Simulation result | Semantic metric and source signals, units, interval, and method |
...
| Layer status | Separate analysis, extraction, simulation-result, and specification state; never one blended pass |
...
2. Run nominal OP evidence first and use a supported result operation to derive
   the simulated bias/headroom facts needed by the topology.
...
preserve lineage from analysis artifacts to simulation results and
specifications. Retain the passing extraction envelope beside downstream
results: the extraction assertion verifies the native binding, while the
immutable result-operation profiles continue to label embedded native lineage
as `unverified`.
...
circuit.simulate
  ...
  -> experimental result.osc.measure
     (no specification.evaluate/v1alpha1 oscillator-receipt adapter)
```
...
credible simulation result. Report the bind failure instead.
...
result operation; a result-operation pass advances to specification evaluation
only if the row already carries a bound. Stop at the first unknown, unavailable
...
- Invoke `$openada:assess-pvt-and-yield` only after nominal analyses,
  simulation-result definitions, and specification limits are frozen.
...
|---|---|
| Analysis evidence pass | Request only supported result operations bound to that exact evidence |
...
| Result-operation pass | Evaluate an explicit specification if one exists |
| Result-operation fail or unknown | Repair the result-operation definition or evidence; do not classify circuit performance |
...
5. simulation results and specification results without collapsing their statuses;
