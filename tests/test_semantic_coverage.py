...
def test_audit_emits_the_complete_deterministic_release_matrix() -> None:
    ...
    assert payload["inventory"] == {
        ...
        "builtin_provider_mapping_count": 14,
        "cli_leaf_count": 33,
        "preflight_assertion_count": 8,
        "profile_count": 16,
        "profile_feature_count": 65,
        "provider_mapping_count": 15,
        "shipped_provider_capability_count": 1,
        "shipped_provider_manifest_count": 1,
        "surface_count": 33,
    }
    assert payload["summary"]["row_count"] == 279
...
def test_untouched_hidden_variant_stays_hidden_without_issues() -> None:
    ...
    # The cosim sibling selector is promoted to experimental (visible, still no
    # release obligation because policy.active_lifecycles is [active]).
    cosim_row = next(
        row
        for row in payload["rows"]
        if row["row_id"]
        == "surface-variant|openada.surface/cli.simulate/v1|behavioral-blocks-cosim"
    )
